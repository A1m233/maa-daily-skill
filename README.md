...
同一环境已完成受控真实 smoke：除 `StartUp` 和保守 `Award` 外，还覆盖了公招容量限制、信用商店严格白名单购买、基建总览批量收取、按星期选择关卡和不使用理智药/源石的停止行为。该结果证明这些特定路径曾跑通，也暴露并固化了 Custom 任务与 Windows 日志的边界；不承诺其他模拟器、账号状态、客户端版本、分辨率或日常组合自动兼容。
