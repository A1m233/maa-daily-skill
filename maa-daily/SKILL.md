...
8. **执行一次。** 使用当前版本实际支持的命令运行一次 task。让普通非交互命令保持 pending 直到退出、超时、取消或运行时失联；不要用高频 Agent/LLM 轮询观察长任务，也不要因暂时零输出重复启动同一任务。Agent 启动了模拟器时，按用户选择在任务后保持可见、恢复可见或正常关闭，并报告实际生命周期；不要遗留一个无窗口但会拦截后续启动的隐藏实例。
