...
3. **运行该账号业务并收尾。** 已接入时先读 [daily-execution.md](references/daily-execution.md)，再调用 `daily_run.py run`；它只编排单账号业务，不启动模拟器、不准备 ADB、不切号。`--account` 是报告别名，`preflight` 和运行中的连接失败保护都不代替上述前置。当前账号收尾并满足继续条件后，才切下一个账号。

**证据复用边界：** 安装位置、已确认配置和用户偏好可以复用；当前设备就绪、无并发操作和账号身份必须有本轮仍有效的证据。同一轮已取得且未失效的证据不重复采集；重启、断连、切号或其他操作者介入后，重新核验受影响状态。旧会话成功、已知端口或更新/重载 Skill 均不能证明这些当前状态。

**连接失败后：** 确认原进程终态并保留已完成业务，从环境步骤排查未启动、启动未完成、端口变化或连接故障；修复具体原因并重新核验受影响前置后，再按[恢复边界](references/daily-execution.md#汇报与恢复)决定补跑范围。不要用公招等业务探测连接，也不要仅更新 Skill 后原样重跑。目标、授权或故障仍不明确时停止，不盲扫端口或连续重试。只读请求不进入这条真实执行链。
...
默认以执行器结束时生成的[用户简报](references/daily-summary.md)作为最终回复正文；多账号按该页调用 `daily_report.py`，传入本次应跑的完整账号列表汇总，不从详细 JSON 另写成功清单。保留简报的结论、待关注项和账号差异，再补充必要的外层事实（如模拟器状态）；未接入执行器或只有分段结果时按该页格式如实说明缺项。技术证据保存在本地，用户排障时再展开。公招保留与其它组件提醒必须合并，奖励已领不代表整轮无待办。
...
- 安装发现、配置创建等工作按实际需要执行，已有有效结果可复用，当前版本或宿主提供等价能力时可替换实现方式；但不能省略[真实运行的入口顺序](#真实运行的入口顺序)要求的前置核验。无需重复动作不等于无需当前证据。
