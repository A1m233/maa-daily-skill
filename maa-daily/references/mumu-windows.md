...
- 启动前先检查已有目标实例，可用时直接复用。需要打开时，选择宿主提供的正常桌面应用启动能力，并确认目标应用不会随任务完成、取消或宿主退出被清理。宿主提供 `launch_desktop_application` 时，传入已发现的 exe 绝对路径、独立参数数组及用途；不要使用会在收尾时终止进程树的受管命令来启动 MuMu。其他宿主使用具有同等生命周期保证的能力，不依赖某个工具名或 shell 启动语法推断保证。
- 系统接受打开请求只代表请求已提交，仍须完成下述四层就绪核验。启动结果为 `unknown` 或回执丢失时，先检查现有实例，不直接重放启动。宿主缺少正常打开能力时说明本次自动启动的缺口，可以由用户正常打开后继续检查和运行 MAA；不要自行尝试脱离进程管理的绕路命令。
