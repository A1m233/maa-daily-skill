...
2026-08-19 的真实双账号运行中，CE-6 单场 36 理智、`series = 6` 被计算为整批 216；两个账号均为 205/205 理智，日志三次记录 `FightTimes.times_finished = 0`，最终 Fight 仍显示 `Completed`，且没有使用理智药或源石。若目标是可靠地耗到不足单场，保守基线使用 `series = 1`、较大的 `times` 上限以及零药/零源石。

同日的恢复 smoke 验证了“高倍率 Fight + 同关卡单倍 Fight”这一原生组合：两个 205/205 的账号都先以 CE-6 `series = 5` 实际完成 5 场、消耗 180 理智，再以 `series = 1` 检查收尾；因剩余 25 小于单场 36，收尾实际为 0 场，两个账号最终均为 25/205，药和源石动作均为 `DoNothing`。2026-08-21 的双账号完整日常又验证了 AP-5：单场 30 理智，两个账号均以 `series = 6` 实际完成 6 场、从 205 降至 25，再由 `series = 1` 正常以 0 场收尾；全程同样没有使用理智药或源石。这能减少进入与结算次数，但不是通用固定倍率：高倍率必须按目标关卡单场消耗与账号最大/常见理智选择，且仍需单倍 task 兜底；无法证明高倍率始终可支付时，公开模板继续使用 `series = 1`。两个 task 都可能在理智不足时短暂进入并关闭恢复界面，summary 的 `Completed` 仍要由各自 `FightTimes` 区分实际场次。
