...
- 奖励检查入口负责独立导航到任务页，不以 Award 完成推断页面；开跑前核对导航与扫描资源部署，以及每个账号检查后再切号的顺序。当前页面无需在预检时就在任务页，不为预检启动扫描。
...
1. **理智与资源目标。** 对照本次目标核对终态理智、实际结算次数及资源动作。清体力要求余额不足目标关卡单场消耗，且授权恢复预算已按要求处理；使用[清体力组件](daily-checks.md#清体力日常)校验读数和计算，不用 Fight Completed 代替目标。非清体力任务按自身次数或材料目标判断。
   使用统一清体力入口时，无论用不用临期药，都按[临期药流程](expiring-medicine.md)核对收尾检测；实际用药时额外报告已核验消耗与剩余库存证据。`detected` 要提醒，`unknown` 也要提醒；“当前理智不足一场”不能代替“临期药已用完”，预算停止不可报成用药目标完成。
...
公招结果使用[公招提取与用户简报](daily-summary.md)；保留标签和未确认开招的组不能隐藏在成功次数中。各组件提醒取并集，不用奖励检查的局部无提醒覆盖整轮缺口。
...
以下信息必须在闭环对账中核验和保留；新建/迁移配置或诊断请求时向用户说明。普通日常默认按[用户简报](daily-summary.md)输出，不逐次重复技术对账全文：
